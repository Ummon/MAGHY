...
Pour ce qui est de la cadence de rotation, il existe plusieurs avis. Certains disent qu’il faut changer tous les jours, d’autre chaque semaine et voir certain à chaque fin de projet. Il faut trouver la
cadence qui convient au mieux à celle de l’équipe.

\subsection{Review programming}

Le concept du review programming est de demander à une ou plusieurs personnes de relire votre code et de donner des critiques constructives dessus. C’est une alternative si votre manager n’est pas convaincu par le pair programming. Mais il se peut aussi que les reviewers ne soient pas très
impliqué et ne vous donne qu’un avis général. Le review programming a aussi une sorte de latence. Lorsqu’on fait des critiques après que le code soit écrit, cela peut être moins bien perçu par le codeur. Surtout quand c’est votre patron. Alors qu’en pair programming, on fait les remarques à chaud, ce qui diminue l’impact de « tu n’as pas bien fait les choses ». Un des avantage de review, c’est qu’on a pas besoin d’être dans la même pièce pour faire la review correctement.
